    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6 flex flex-col hover:shadow-md transition-all duration-200">
      ...
      <div className="flex gap-2">
        <a href={tool.affiliateUrl} target="_blank" rel="nofollow sponsored"
          className="flex-1 text-center text-sm font-semibold py-2.5 rounded-xl transition-all text-white"
          style={{ background: brand?.bg ?? '#4F46E5' }}>
          {'Visiter le site →'}
        </a>
    ...
    <div className="bg-white rounded-xl p-5 border border-gray-100 flex flex-col sm:flex-row items-start sm:items-center gap-4">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {tool.freeTrial && (
            <span className="text-xs text-emerald-600 font-medium bg-emerald-50 px-2 py-0.5 rounded-full">✓ {tool.freeTrial}</span>
          )}
        </div>
      </div>
      <div className="flex flex-col gap-1.5 shrink-0 w-full sm:w-auto">
        <a href={tool.affiliateUrl} target="_blank" rel="nofollow sponsored"
          className="text-center text-sm font-semibold py-2.5 px-5 rounded-lg text-white transition-opacity hover:opacity-90"
          style={{ background: brand?.bg ?? '#4F46E5' }}>
          {'Visiter le site →'}
        </a>
