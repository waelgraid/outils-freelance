...
const reviews = [
  { name: 'Thomas L.', job: 'Plombier auto-entrepreneur, Lyon', text: 'Avant de trouver ce site, je perdais des heures à comparer des logiciels. En 10 minutes j\'avais ma réponse. Merci !', tool: 'Abby' },
  { name: 'Sophie M.', job: 'Consultante RH freelance, Paris', text: 'Le calculateur URSSAF m\'a évité une mauvaise surprise de 800€. Et le quiz m\'a orientée vers Freebe que je ne connaissais pas.', tool: 'Freebe' },
  { name: 'Karim B.', job: 'Développeur freelance, Bordeaux', text: 'Comparatifs honnêtes, les inconvénients sont mentionnés. C\'est rare et ça donne confiance.', tool: 'Indy' },
]
...
export default function HomePage() {
  return (
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 pt-16 pb-10 text-center">
          ...
          <div className="inline-flex items-center gap-6 sm:gap-10 bg-white border border-gray-100 shadow-sm rounded-2xl px-8 py-4 mb-10 flex-wrap justify-center">
            {[
              { value: '+50', label: 'outils testés' },
              { value: '9', label: 'sélectionnés' },
              { value: '32', label: 'guides pratiques' },
      ...
      <section className="max-w-5xl mx-auto px-4 sm:px-6 py-16">
        <div className="text-center mb-10">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Ce qu&apos;en disent nos lecteurs</h2>
          <p className="text-gray-400 text-sm">Des indépendants qui ont trouvé leurs outils grâce à nos comparatifs</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {reviews.map(r => (
            <div key={r.name} className="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-gray-400">{r.job}</div>
                </div>
                <span className="text-xs font-semibold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">→ {r.tool}</span>
              </div>
            </div>
          ))}
        </div>
