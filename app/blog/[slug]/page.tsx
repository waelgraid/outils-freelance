...
function renderContent(content: string) {
  ...
  // Convert markdown inline formatting to HTML including links
  function parseInline(text: string): string {
    return text
      .replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>')
      .replace(/\*(.*?)\*/g, '<em>$1</em>')
      .replace(/\[([^\]]+)\]\((\/[^)]+)\)/g, '<a href="$2" class="text-indigo-600 hover:text-indigo-800 underline font-medium">$1</a>')
      .replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, '<a href="$2" target="_blank" rel="nofollow noopener" class="text-indigo-600 hover:text-indigo-800 underline font-medium">$1</a>')
  }
    ...
    if (line.startsWith('## ')) {
      ...
      elements.push(
        <ul key={key++} className="list-disc pl-6 mb-4 space-y-1.5">
          {items.map((item, j) => <li key={j} className="text-gray-700 text-sm leading-relaxed" dangerouslySetInnerHTML={{ __html: parseInline(item) }} />)}
        </ul>
      )
    } else if (line.startsWith('|')) {
                ...
                <tbody>
                  {tableRows.slice(1).map((row, ri) => (
                    <tr key={ri} className="hover:bg-gray-50">
                      {row.map((cell, ci) => <td key={ci} className="px-4 py-2 border border-gray-200 text-gray-700" dangerouslySetInnerHTML={{ __html: parseInline(cell) }} />)}
                    </tr>
                  ))}
                </tbody>
              </table>
...
export default function ArticlePage({ params }: { params: { slug: string } }) {
  ...
  const comparatifLink = (() => {
    ...
    if (text.match(/comptab|urssaf|cotisation|déclaration/)) return '/comparatifs/logiciels-facturation'
    if (text.match(/gestion|devis-artisan|meilleur-logiciel-gestion/)) return '/comparatifs/logiciels-facturation'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-2">
          ...
          <div className="flex items-center gap-3 text-xs text-gray-400 mb-8 pb-6 border-b border-gray-100">
            ...
            <span>·</span>
            <Link href="/methodologie" className="hover:text-indigo-600 transition-colors">
              Par {AUTHOR.name}
            </Link>
          </div>
          ...
          <div className="card p-5 sticky top-20">
            ...
            {isMutuelle ? (
              <div>
                ...
                {[
                  { name: 'Alan', desc: 'La mutuelle 100% digitale', prix: 'À partir de 39€/mois', slug: 'alan', url: 'https://alan.com' },
                  { name: 'Wemind', desc: 'La moins chère pour freelances', prix: 'À partir de 22€/mois', slug: 'wemind', url: 'https://www.wemind.io' },
                ].map(m => (
