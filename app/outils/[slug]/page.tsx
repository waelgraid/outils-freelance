...
const TOOL_BG: Record<string, string> = {
  ...
  abby: '#00C48C', wemind: '#7C3AED', april: '#E11D48',
}
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="card p-8 mb-8">
          <div className="flex flex-col sm:flex-row items-start gap-6">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-gray-500 mt-1">Note éditoriale selon <Link href="/methodologie" className="underline hover:text-indigo-600">notre grille de notation</Link> — pas une moyenne d&apos;avis clients</p>
            ...
            <div className="shrink-0 text-center sm:text-right">
              ...
              {tool.freeTrial && (
                <div className="text-xs text-emerald-600 font-medium mb-4 inline-block">✓ {tool.freeTrial}</div>
              )}
              <a href={tool.affiliateUrl} target="_blank" rel="nofollow sponsored" className="btn-primary block text-center">
                Voir le site →
              </a>
              <p className="text-xs text-gray-500 mt-2">Lien affilié · sans surcoût</p>
            </div>
          </div>
        </div>
        ...
        {/* VerdictBox final — point de conversion principal */}
        {review && (
          <VerdictBox
            slug={tool.slug}
            name={tool.name}
            affiliateUrl={tool.affiliateUrl}
            score={review.score.note}
            verdictShort={review.verdictShort}
            pros={tool.pros.slice(0, 4)}
            trialLabel={tool.freeTrial ?? ''}
          />
        )}
      </div>
    </>
